...
namespace MyDBMS
{
    public partial class tableWORKER : Form
    {
        public static tableWORKER w2;//静态成员用于对外调用
        string id = "NULL";
        public tableWORKER()
        {
            InitializeComponent();
        ...
        private void tableWORKER_Load(object sender, EventArgs e)
        {
            W_Show();//初始查询
        }
        public void W_Search(string sql)
        {
        ...
        public string W_id()//返回选中的元组ID，因为删除操作需要通过候选键对元组进行定位
        {
            return id;
        }
    }
